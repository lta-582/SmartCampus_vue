...
// 用户管理-修改发布状态 111
export const userLock = (params) => { return axios.get("/api/user/publish?userId=" + params.userId + "&isPublish=" + params.isPublish + "&token=" + localStorage.getItem('logintoken')) };

// 用户管理-修改激活状态 111
export const userActive = (params) => { return axios.get("/api/user/active?userId=" + params.userId + "&isActive=" + params.isActive + "&token=" + localStorage.getItem('logintoken')) };
...
// 文章管理-获取官方动态列表 11
export const sActiveList = (params) => { return req("post", "/api/schoolArticle/list", params) };
// 文章管理-保存（添加编辑）11
export const sActiveSave = (params) => { return req("post", "/api/schoolArticle/save", params) };
// 文章管理-删除文章11
export const sActiveDelete = (params) => { return axios.delete("/api/schoolArticle/delete?ids=" + params + "&token=" + localStorage.getItem('logintoken')).then(res => res.data) };

// 文章管理-获取评论列表 11
export const getCommentList = (params) => { return req("post", "/api/comment/list", params) };
// 文章管理-删除评论11
export const commentDelete = (params) => { return axios.delete("/api/comment/delete?ids=" + params + "&token=" + localStorage.getItem('logintoken')).then(res => res.data) };

// 轮播图管理-获取轮播图列表 11
export const imageList = (params) => { return req("post", "/api/image/list", params) };
// 轮播图管理-保存（添加编辑）11
export const imageSave = (params) => { return req("post", "/api/image/save", params) };
// 轮播图管理-删除文章11
export const imageDelete = (params) => { return axios.delete("/api/image/delete?ids=" + params + "&token=" + localStorage.getItem('logintoken')).then(res => res.data) };
...
// 用户管理-刷新缓存 111
export const userFlashCache = (params) => { return req("get", "/api/user/flashCache/?userId=" + params, {}) };
